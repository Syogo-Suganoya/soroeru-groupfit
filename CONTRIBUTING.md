...
**判定の数値はドメインの純関数で決めます。** LLM には言語化と自由記述の解釈だけをさせます。
...
どちらも `CompositorPort` / `VideoPort` の裏にあり、実装は Pillow で完結しています。
会場ライティングは `figure.apply_lighting` の色調調整による近似で、物理的な再照明ではありません。

**外部の生成モデルに差し替えるときも、誰をシルエットにするかの判断はローカル側に残します。**
渡してよいのは「ローカルで組み立てたベース画像」までで、未同意の人の顔を生成させる余地を作りません。
合成結果がどのエンジンで作られたかは `PreviewRevision.engine` / `Movie.engine` に残します。
