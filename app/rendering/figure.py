...
試着と集合プレビューをローカルだけで成立させるための可視化。
実際の試着エンジンに差し替えるときは、このモジュールは
集合プレビューの「並べ方」だけを担う。

同意していないメンバーは必ずシルエット（顔なし・グレー）で描く。
描画側でも同意フラグを見ることで、合成の不変条件を二重に守る。
"""
...
def tone_match_score(tone: str, color_hex: str) -> float:
    """パーソナルカラーと衣装色の適合度（0-1）。決定的な近似。

    各シーズンの代表色相との距離で評価する。あくまで提示順の並び替えに使う目安で、
    「似合う / 似合わない」を断定するものではない。
    """
